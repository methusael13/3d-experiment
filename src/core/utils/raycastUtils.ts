/**
 * Raycasting utilities for 3D picking and intersection tests
 */

import { mat4 } from 'gl-matrix';

/**
 * Camera interface for raycasting (minimal required methods)
 */
 ...
 */
export function normalize(v: [number, number, number]): void {
 ...
 */
export function getObjectRadius(obj: { scale: [number, number, number] }): number {
...
export function projectToScreen(
  ...
  const pos4: [number, number, number, number] = [worldPos[0], worldPos[1], worldPos[2], 1];
...
export function screenToRay(
  ...
  const nearPoint: [number, number, number, number] = [ndcX, ndcY, -1, 1];
  const farPoint: [number, number, number, number] = [ndcX, ndcY, 1, 1];
  
  const worldNear = transformPoint(nearPoint, invVPMatrix);
  const worldFar = transformPoint(farPoint, invVPMatrix);
  
  const rayOrigin: [number, number, number] = [worldNear[0], worldNear[1], worldNear[2]];
 ...
 * @returns Intersection point or null if ray is parallel/behind
 */
 ...
 * @returns Hit position [x, z] or null if no hit
 */

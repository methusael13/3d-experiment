/**
 * volumetric-fog-apply.wgsl — Post-process: apply froxel fog to scene
 *
 * Fullscreen pass that reads the integrated froxel 3D texture and composites
 * accumulated in-scattered light + transmittance with the scene color.
 *
 * Replaces AtmosphericFogEffect when volumetric fog is active.
 */
...
fn linearizeDepthReversedZ(depth: f32, near: f32, far: f32) -> f32 {
  // Sky pixels in reversed-Z have depth == 0.0
